...
def read_vlan(vlan_id: int, db: Session = Depends(get_db)):
    vlan = crud.get_vlan_by_id(db, vlan_id=vlan_id)
    if vlan is None:
...
def update_vlan_endpoint(vlan_id: int, db: Session = Depends(get_db)):
    # Check if customer_id is anything but 0 and if so, check if customer exists
    if vlan.customer_id != 0:
        customer = crud.get_customer(db, vlan.customer_id)
        if not customer:
    ...
    vlan = crud.update_vlan_by_id(db, vlan_id=vlan_id)
    return vlan
...
def delete_vlan_endpoint(vlan_id: int, db: Session = Depends(get_db)):
    ...
    vlan = crud.delete_vlan_by_id(db, vlan_id=vlan_id)
    return vlan
