...
def _overlay_stretches_for_sites(db: Session, sites: list[Site]) -> list[dict[str, Any]]:
    ...
    rows = []
    for site in sites:
        for row in fac_svc.list_overlay_stretches(db, site_id=site.id):
            if row.id in seen:
                continue
            seen.add(row.id)
            rows.append(row)
    return ipam_sync._overlay_stretches_export(db, rows)
...
def _ipam_for_site(db: Session, site: Site) -> dict[str, Any]:
    ...
    return {
        ...
        "overlay_stretches": [
            {
                ...
                "a_overlay_slug": s.get("a_overlay_slug"),
                "z_site_slug": s.get("z_site_slug"),
                "z_overlay_slug": s.get("z_overlay_slug"),
        ...
        "tunnels": [
            {
                "vpn_slug": t.get("vpn_slug"),
                "name": t.get("name"),
                "slug": t.get("slug"),
                "status": t.get("status"),
            }
            for t in raw.get("tunnels") or []
            if t.get("vpn_slug") and t.get("slug")
        ],
        "tunnel_transports": [
            {
                "vpn_slug": t.get("vpn_slug"),
                "tunnel_slug": t.get("tunnel_slug"),
                "circuit_number": t.get("circuit_number"),
            }
            for t in raw.get("tunnel_transports") or []
            if t.get("vpn_slug") and t.get("tunnel_slug") and t.get("circuit_number")
        ],
        "autonomous_systems": [
...
def _export_device_interface_vlan_members(
    db: Session,
    *,
    devices: list[DeviceInstance],
    site_by_id: dict[int, Site],
    ...
    members = (
        list(
            db.execute(
                select(DeviceInterfaceVlanMember).where(DeviceInterfaceVlanMember.interface_id.in_(iface_by_id))
            ).scalars().all()
        )
        if iface_by_id
        else []
    )
    vlan_ids = {m.ipam_vlan_id for m in members}
    vlan_by_id = {
        v.id: v
        for v in (db.execute(select(IpamVlan).where(IpamVlan.id.in_(vlan_ids))).scalars().all() if vlan_ids else [])
    }
    return {
        "device_interface_vlan_members": [
            {
                ...
                "vlan_slug": vlan_by_id[m.ipam_vlan_id].slug if m.ipam_vlan_id in vlan_by_id else None,
                "role": m.role,
            }
            for m in members
...
def _export_device_port_interfaces(
    db: Session,
    *,
    devices: list[DeviceInstance],
    site_by_id: dict[int, Site],
    ...
    device_ids = {d.id for d in devices}
    ports = (
        list(db.execute(select(DevicePort).where(DevicePort.device_id.in_(device_ids))).scalars().all())
        if device_ids
        else []
    )
    iface_ids = {p.interface_id for p in ports if p.interface_id}
    iface_by_id = {
        i.id: i
        for i in (
            db.execute(select(DeviceInterface).where(DeviceInterface.id.in_(iface_ids))).scalars().all()
            if iface_ids
            else []
        )
    }
        ...
        "device_port_interfaces": [
            {
                "device_name": device_by_id[p.device_id].name if p.device_id in device_by_id else None,
                "site_slug": (
                    site_by_id[device_by_id[p.device_id].site_id].slug
                    if p.device_id in device_by_id
                    and device_by_id[p.device_id].site_id
                    and device_by_id[p.device_id].site_id in site_by_id
                    else None
                ),
                "port_kind": p.kind,
                "port_name": p.name,
                "interface_name": iface_by_id[p.interface_id].name if p.interface_id in iface_by_id else None,
            }
            for p in ports
...
def _export_device_interface_vrfs(
    db: Session,
    *,
    devices: list[DeviceInstance],
    site_by_id: dict[int, Site],
    ...
    vrf_ids = {i.ipam_vrf_id for i in ifaces if i.ipam_vrf_id}
    vrf_by_id = {
        v.id: v
        for v in (db.execute(select(IpamVrf).where(IpamVrf.id.in_(vrf_ids))).scalars().all() if vrf_ids else [])
    }
    return {
        "device_interface_vrfs": [
            {
                ...
                "interface_name": i.name,
                "vrf_slug": vrf_by_id[i.ipam_vrf_id].slug if i.ipam_vrf_id in vrf_by_id else None,
            }
            for i in ifaces
            if i.device_id in device_by_id and i.ipam_vrf_id in vrf_by_id
        ],
    }
