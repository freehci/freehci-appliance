...
def _vrf_stretches_export(db: Session, stretches: list) -> list[dict[str, Any]]:
    out: list[dict[str, Any]] = []
    for row in stretches:
        low = fac_svc.get_vrf(db, row.vrf_low_id)
        high = fac_svc.get_vrf(db, row.vrf_high_id)
        sites = {}
        ...
        out.append(
            {
                ...
                "a_vrf_slug": low.slug if low is not None else None,
                "z_site_slug": sites[high.site_id].slug if high is not None and high.site_id in sites else None,
                "z_vrf_slug": high.slug if high is not None else None,
...
def _vlan_stretches_export(db: Session, stretches: list) -> list[dict[str, Any]]:
    out: list[dict[str, Any]] = []
    for row in stretches:
        low = fac_svc.get_vlan(db, row.vlan_low_id)
        high = fac_svc.get_vlan(db, row.vlan_high_id)
        sites = {}
        ids = {x.site_id for x in (low, high) if x is not None}
        if ids:
            sites = {s.id: s for s in db.execute(select(Site).where(Site.id.in_(ids))).scalars().all()}
        out.append(
            {
                ...
                "name": row.name,
                "a_site_slug": sites[low.site_id].slug if low is not None and low.site_id in sites else None,
                "a_vlan_slug": low.slug if low is not None else None,
                "z_site_slug": sites[high.site_id].slug if high is not None and high.site_id in sites else None,
                "z_vlan_slug": high.slug if high is not None else None,
                "description": row.description,
            }
        )
    return out
...
def export_site(db: Session, site_id: int) -> dict[str, Any]:
    ...
    vrf_stretches = fac_svc.list_vrf_stretches(db, site_id=site_id)
    ...
    return {
        ...
        "vrf_stretches": _vrf_stretches_export(db, vrf_stretches),
        ...
        "circuits": [
            {
                ...
                "ownership": getattr(c, "ownership", None),
            }
            for c in circuits
        ],
